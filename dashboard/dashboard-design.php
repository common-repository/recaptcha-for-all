...
if (isset($_GET["page"]) && $_GET["page"] == "recaptcha_for_all_admin_page") {
    if (
        isset($_POST["process"]) &&
        sanitize_text_field($_POST["process"]) == "recaptcha_for_all_admin_page_background"
    ) {
        if (
            isset($_POST["recaptcha_my_plugin_nonce"]) &&
            wp_verify_nonce(
                sanitize_text_field($_POST["recaptcha_my_plugin_nonce"]),
...
<big>
    ...
    <form class="recaptcha_for_all-form" method="post" action="admin.php?page=recaptcha_for_all_admin_page&tab=design">
       <?php echo '<input type="hidden" name="recaptcha_my_plugin_nonce" value="' . esc_attr( $recaptcha_my_plugin_nonce ) . '" />'; ?>
       ...
       <div style="border: 1px solid #999; border-radius: 5px; padding: 0px 10px 10px 10px; margin-bottom: 20px;">
        ...
        <input type="color" id="recaptcha_foreground" name="recaptcha_foreground" value="<?php esc_attr(
            $recaptcha_for_all_foreground_color
        ); ?>">
        <br><br>
        <?php esc_attr_e("Background Color:", "recaptcha-for-all"); ?>
        <input type="color" id="recaptcha_background" name="recaptcha_background" value="<?php esc_attr(
            $recaptcha_for_all_background_color
        ); ?>">
        <br><br>
        <?php esc_attr_e("Button Text Color:", "recaptcha-for-all"); ?>
        <input type="color" id="recaptcha_btn_foreground" name="recaptcha_btn_foreground" value="<?php esc_attr(
            $recaptcha_for_all_btn_foreground_color
        ); ?>">
        <br><br>
        <?php esc_attr_e("Button Background Color:", "recaptcha-for-all"); ?>
        <input type="color" id="recaptcha_btn_background" name="recaptcha_btn_background" value="<?php esc_attr(
            $recaptcha_for_all_btn_background_color
        ); ?>">
</div>
        <br>
...
foreach ($background_images as $image): ?>
    <label style="display: inline-block; text-align: center; width: 16%;">
        ...
        <div style="text-align: center;">
            <input type="radio" id="radio_<?php echo esc_attr($image); ?>" name="recaptcha_for_all_image_background" value="<?php echo esc_attr($image); ?>" <?php echo $recaptcha_for_all_image_background === $image ? "checked" : ""; ?> style="width: 16px; height: 16px;">
        </div>
    </label>
<?php endforeach; ?>
?>
