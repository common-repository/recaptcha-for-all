...
$x = rand(1, 2);
if ($x < 2) {
    echo '<h2>' . esc_attr__("Did you like the Recaptcha For All Plugin?", "recaptcha-for-all") . '</h2>';
    echo '<img src="' . esc_url(RECAPTCHA_FOR_ALL_IMAGES) . '/help3.jpg' . '" style="width: 100%; height: auto;" />';
    esc_html_e('Please support us by rating our plugin on WordPress.org. Help us keep this plugin live and updated.', 'recaptcha-for-all');
?>
    <br /><br />
    <a href="https://wordpress.org/support/plugin/recaptcha-for-all/reviews/#new-post" class="button button-medium button-primary"><?php esc_attr_e('Rate', 'recaptcha-for-all'); ?></a>
<?php
} else {
    echo '<h2>' . esc_attr__("Please help us keep the plugin live & up-to-date", "recaptcha-for-all") . '</h2>';
    echo '<img src="' . esc_url(RECAPTCHA_FOR_ALL_IMAGES) . '/help1.jpg' . '" style="width: 100%; height: auto;" />';

    esc_attr_e('If you use & enjoy Recaptcha For All Plugin, please rate it on WordPress.org. It only takes a second and helps us keep the plugin live and maintained. Thank you!', 'recaptcha-for-all');
?>
    <br />
    <a href="https://wordpress.org/support/plugin/recaptcha-for-all/reviews/#new-post" class="button button-medium button-primary"><?php esc_attr_e('Rate', 'recaptcha-for-all'); ?></a>
<?php
...
if ($cached_coupon_data !== '' && $cached_coupon_data !== false) {
    ...
    echo '<img src="' . esc_url(RECAPTCHA_FOR_ALL_IMAGES) . '/' . esc_attr($image) . '" style="width: 100%; height: auto;" />';
    echo "<br>";
    ...
    if ($title && $message_text) {
        echo '<ul>';
        echo '<h2>' . esc_html($title) . '</h2>';
        echo '<img src="' . esc_url(RECAPTCHA_FOR_ALL_IMAGES) . '/news.gif" style="width: 100%; height: auto;" />';
        echo "<br>";
