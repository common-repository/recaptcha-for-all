...
if (isset($_GET['page']) && $_GET['page'] == 'recaptcha_for_all_admin_page') {
    if (isset($_POST['process']) && sanitize_text_field($_POST['process']) == 'recaptcha_for_all_admin_page_keys') {
        //get limit
...
if (empty($recaptcha_for_all_settings_provider)) {
    echo '<p class="description">'.esc_attr__("Provider not selected. Go to the General Settings tab and choose Google or Turnstile.", "recaptcha-for-all");
    echo '</div>';
    return;
} else {
    echo '<p class="description">'.esc_attr__("Provider currently selected", "recaptcha-for-all");
    echo ': '.esc_attr($recaptcha_for_all_settings_provider);
    echo ' ';
    echo '<p class="description">'.esc_attr__("Go to the General Settings tab to change it.", "recaptcha-for-all");
}
echo '<br></p>';
//
...
echo '<input class="recaptcha_for_all-submit button-primary" type="submit" value="'.esc_attr__("Update", "recaptcha-for-all").'" />';
    ...
    echo '<br />';
    echo '<br />';
    echo '<input class="recaptcha_for_all-submit button-secondary" type="submit" id="recaptcha_for_all_test_keys" value="'.esc_attr__("Test Keys", "recaptcha-for-all").'" style="display: block;" />';


    if($recaptcha_for_all_settings_provider == 'turnstile')
    {
       // echo esc_attr__("Click before in verify box below","recaptcha-for-all");
    }
//
